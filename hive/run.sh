...
run_migrations(){
  if $HIVE_HOME/bin/schematool -dbType "$DATABASE_TYPE" -validate | grep 'Done with metastore validation' | grep '[SUCCESS]'; then
    echo 'Database OK'
    return 0
  else
    # TODO: how to apply new version migrations or repair validation issues
    $HIVE_HOME/bin/schematool --verbose -dbType "$DATABASE_TYPE" -initSchema
  fi
}
